...
app.post("/api/quotes", function(req, res) {
  ...
  connection.query("INSERT INTO quotes (author, quote) values (?,?)", [req.body.author, req.body.quote], (err, data) => {
    if (err) {
      res.status(500).end()
    }
...
app.delete("/api/quotes/:id", function(req, res) {
  ...
  connection.query("DELETE from quotes WHERE id = ?", [req.params.id], (err, data) => {
    if (err) {
      res.status(500).end()
    }
...
app.put("/api/quotes/:id", function(req, res) {
  ...
  connection.query("UPDATE quotes set author = ?, quote = ? where id = ?", [req.body.author, req.body.quote, req.params.id], (err, data) => {
    if (err) {
      res.status(500).end()
    }
